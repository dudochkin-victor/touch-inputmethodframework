...
class CompositingWidgetConfigurator
    : public MImAbstractWidgetVisitor
{
public:
    ...
    if (plugin) {
        if (!plugin->supportedStates().isEmpty()) {
            ...
            if (im) {
                ...
                MIMPluginManagerPrivate::configureWidgetsForCompositing(centralWidget.data());
            } else {
...
void MIMPluginManagerPrivate::configureWidgetsForCompositing(QWidget *widget)
{
    CompositingWidgetConfigurator configurator;
    MIMApplication::visitWidgetHierarchy(&configurator, widget);
}
